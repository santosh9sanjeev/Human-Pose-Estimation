...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=3, padding=1, stride=1, bias=True)
...
class Hourglass(nn.Module):
    ...
    def _hour_glass_forward(self, n, x, hg_prev):
        up1 = self.hg[n-1][0](x)
        
        # if hg_prev is not None and n>0:
        #     # for jj in range(len(hg_prev)):
        #     #     print(hg_prev[jj].shape)
        #     up1 += hg_prev[self.depth-n]
        # elif n>0:
        #     self.upconvs.append(up1)
        
        low1 = F.max_pool2d(x, 2, stride=2)
        low1 = self.hg[n-1][1](low1)

        if n > 1:
            low2 = self._hour_glass_forward(n-1, low1, hg_prev)
        else:
    ...
    def forward(self, x, hg_prev):
        res = self._hour_glass_forward(self.depth, x, hg_prev)
        return res, self.upconvs
